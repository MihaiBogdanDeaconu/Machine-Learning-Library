...
public class Accuracy<F, L extends Number> implements EvaluationMeasure<F, L> {
    private L trueLabel;
    private L falseLabel;

    public Accuracy(L trueLabel, L falseLabel) {
    ...
    public double evaluate(List<Instance<F, L>> instances, List<L> predictions){
        ...
        return ((double) tp + (double) tn) / instances.size();
    }
}
